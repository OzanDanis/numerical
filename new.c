...
int main() {
    ...
    do {
        ...
        while (control < 0 || control > 10) {
            printf("\ntYou entered the wrong command, please re-enter: ");
            scanf("%d", &control);
        }
		printf("\n\n");
        ...
        if (control == 1) {
            ...
            for (i = 0; i < size; i++) {
                for (j = 0; j < size + 1; j++) {
                    printf("\nEnter the %dth row, %dth column: ", i + 1, j + 1);
                    scanf("%lf", &matrix[i][j]);
                }
            }
            printf("\nEnter the epsilon:");
        	...
        	printf("\t\t\t\t\t\t!YOU ARE USING NUMERICAL DIFFERENTIATION!\n");
...
void gregory_newton() {
    ...
    double value, sum, u;
    int i, j, n;
	printf("\t\t\t\t\t\t!YOU ARE USING GREGORY NEWTON!\n");
